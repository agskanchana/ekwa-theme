...
function my_acf_init_block_types() {
	...
	$acf_blocks =  get_installed_resources($term_ID);

	//$Blocks = array(array( 'name'=> 'Welcome block', 'slug' => 'services_welcome_content', 'template' => 'template-185'), array( 'name'=> 'Testimonial Test', 'slug' => 'testimonial', 'template' => 'template-191'));

	  if($acf_blocks){
		  foreach($acf_blocks as $block){

		  $blockID = $block['post_id'];
		  ...
		  acf_register_block_type(array(
			  'name'              => $slug,
			  'title'             => __($blockName),
			  'render_template'   => 'template-parts/theme-blocks/'.$template.'/block.php',
			  'category'          => 'formatting',
			  'icon'              => $icon,
