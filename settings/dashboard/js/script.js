...
function template(data) {
  ...
  var downloadBTN = '';
  let elementIndex = -1;
  var installedPostID = -1;
  var installedBlockSLug = '';
  var installedBlockJson = '';
  //console.log(data);
    ...
    if(jQuery.inArray(this.id, installedHeaders) == -1){
      ...
      Visibility = 'd-none-b';
       ...
       installedBlocks.forEach((element, index, array) => {
        ...
        installedPostID = installedBlocks[elementIndex].post_id;
        installedBlockSLug =  installedBlocks[elementIndex].block_slug;
        installedBlockJson =  installedBlocks[elementIndex].json_file;
    }
      ...
      <span class="btn-group-right">
            ${linkOpen}<i class="fas fa-download"></i> Download test${linkClose}
            <a target="_blank" href="${this.acf.thumbnail}" type="button" class="btn btn-secondary btn-sm"><i class="fas fa-eye"></i> Preview</a>
              
            <a data-slug="${installedBlockSLug}" data-file="${installedBlockJson}" class="btn btn-danger deleteItem btn-sm" data-postid="${installedPostID}" data-id="${this.id}" href="javascript:;"><i class="fas fa-trash-alt"></i> Remove</a>
      </span>
      </li>`;
...
$(".deleteItem").click(function(e){*/
    ...
    $('#headers_list').on('click', '.deleteItem', function(e) {
